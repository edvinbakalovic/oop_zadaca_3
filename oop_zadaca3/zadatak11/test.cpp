...
#include <list>
#include <string>
#include <algorithm> // za std::equal
...
TEST_CASE("my::sort sorts a vector of six ints") {
    ...
    my::sort(begin(v), end(v));
    CHECK(std::equal(begin(v), end(v), begin(s)));
}
...
TEST_CASE("my::sort sorts a list of six ints with a predicate function") {
    ...
    // Sortira opadajuće
    my::sort(begin(v), end(v), [](const auto& a, const auto& b){ return a > b; });
    s.reverse(); // zamjena std::reverse za listu
    CHECK(std::equal(begin(v), end(v), begin(s)));
}
...
TEST_CASE("my::sort sorts a vector of four strings with a predicate function") {
    ...
    // Sortira po dužini stringa opadajuće
    my::sort(begin(v), end(v), [](const auto& a, const auto& b){ return a.size() > b.size(); });
    CHECK(std::equal(begin(v), end(v), begin(s)));
}
